...
import bubble_structure2 as bub
#import timeit
...
def zero_delta(delta, full_params):
    ...
    bstrux_result = bub.bstrux(full_params)
...
def zero_beta(beta, full_params):
    ...
    R1 = scipy.optimize.brentq(bub.R1_zero, 1e-3 * my_params["R2"], my_params["R2"],args=([my_params["Lw"], my_params["Eb"], my_params["vw"], my_params["R2"]]), xtol=1e-18)
    Pb = state_eq.PfromE(my_params["Eb"], my_params["R2"], R1)

    # calculate Edot
    Edot = state_eq.beta_to_Edot(Pb, R1, my_params["beta"], my_params)
...
def zeroODE34(x, full_params):
    ...
    # calculate pressure
    R1 = scipy.optimize.brentq(bub.R1_zero, 1e-3 * my_params["R2"], my_params["R2"],
...
def Edot_Tdot(beta, delta, params, verbose=0):
    ...
    R1 = scipy.optimize.brentq(bub.R1_zero, 1e-3 * params["R2"], params["R2"],args=([params["Lw"], params["Eb"], params["vw"], params["R2"]]), xtol=1e-18)
    Pb = state_eq.PfromE(params["Eb"], params["R2"], R1)
    Edot = state_eq.beta_to_Edot(Pb, R1, beta, params)
    Tdot = state_eq.delta_to_Tdot(params["t_now"],params["T0"],delta)
